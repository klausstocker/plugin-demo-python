...
def _get_plugin(typ: str, name: str, config: str):
    plugin = plugin_configuration.create_plugin_service(typ or "", name or "", config or "")
    if plugin is None:
        raise HTTPException(status_code=404, detail=f"Plugin type '{typ}' not found")
    return plugin
...
async def get_html(r: PluginRequestDto) -> str:
    plugin = _get_plugin(r.typ, r.name, r.config)
...
async def configuration_info(r: PluginConfigurationInfoRequestDto) -> PluginConfigurationInfoDto:
    info = plugin_configuration.configuration_info(
        r.typ, r.name, r.config, r.configurationID or "", r.timeout
    )
    if info is None:
...
async def set_configuration_data(r: PluginSetConfigurationDataRequestDto) -> PluginConfigDto:
    result = plugin_configuration.set_configuration_data(
        r.typ, r.configurationID or "", r.configuration or "", r.questionDto
    )
    if result is None:
        raise HTTPException(status_code=404, detail=f"Plugin type '{r.typ}' not found")
    return result
...
async def get_configuration(r: PluginConfigurationRequestDto) -> str:
    result = plugin_configuration.get_configuration(r.typ, r.configurationID or "")
    if result is None:
